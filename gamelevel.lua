...
function scene:show( event )
	local phase = event.phase
      ...
      composer.removeScene(previousScene)
       ...
       invaderTimer =    timer.performWithDelay(1500, fireInvaderBullet,-1)
	   end
end
...
function drawDebugButtons()
	...
	local left = display.newRect(10,700,50,50)
	left.name = "left"
	left:addEventListener("tap", moveEm)
    scene.view:insert(left)
	local right = display.newRect(600,700,50,50)
	right.name = "right"
	scene.view:insert(right)
     left:addEventListener("tap", moveEm)
    right:addEventListener("tap", moveEm)
    
    
    local fire = display.newRect(300,750,50,50)
    fire:addEventListener("tap", firePlayerBullet)
    scene.view:insert(fire)
end

function setupBackgroundRect()
		backgroundRect = display.newRect( 0, 0, display.contentWidth, display.contentHeight)
		backgroundRect:setFillColor( 0,0,0)
		scene.view:insert(backgroundRect)
end
...
function setupInvaders()
	local xPositionStart =display.contentCenterX - 16 - (gameData.invaderNum *42)
	local numberOfInvaders = gameData.invaderNum *2+1 
	for i = 1, gameData.rowsOfInvaders do
		for j = 1, numberOfInvaders do
			local tempInvader = display.newImage("invader1.png",xPositionStart + (42*(j-1)), i * 46 )
...
function onCollision( event )
   ...
   params.thePlayerBullet.isBodyActive = false
